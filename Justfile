...
project := "recon_silly_ation"
OWNER := "hyperpolymath"
REPO := "recon-silly-ation"
version := "0.1.0"
...
build-watch:
    ...
    find src/interface/ffi -name '*.zig' | entr -c just build
...
# Per STANDING rule: P2P + E2E + aspect
...
# Install build artifacts to PREFIX (default /usr/local; override with PREFIX env var)
install: build-release
    @echo "Installing {{project}}..."
    @PREFIX="$${PREFIX:-/usr/local}"; \
    mkdir -p "$$PREFIX/lib" "$$PREFIX/include"; \
    cp -r src/interface/ffi/zig-out/lib/. "$$PREFIX/lib/" 2>/dev/null || echo "  (no lib/ artifacts yet — build first)"; \
    cp -r generated/abi/*.h "$$PREFIX/include/" 2>/dev/null || echo "  (no generated ABI headers yet)"; \
...
deps:
    ...
    @command -v zig >/dev/null 2>&1 && echo "  [OK] zig ($$(zig version))" || echo "  [FAIL] zig not found"
    @command -v idris2 >/dev/null 2>&1 && echo "  [OK] idris2 ($$(idris2 --version))" || echo "  [WARN] idris2 not found (only needed to regenerate ABI headers)"
...
container-init:
    ...
    if [ ! -d "container" ]; then
        ...
        echo "This repo may not have been created from the RSR template scaffold."
        exit 1
    fi
...
doctor:
    @echo "Running diagnostics for recon-silly-ation..."
...
help-me:
    @echo "=== recon-silly-ation Help ==="
    @echo "Platform: $(uname -s) $(uname -m)"
    @echo "Shell: $SHELL"
    @echo ""
    @echo "To report an issue:"
    @echo "  https://github.com/hyperpolymath/recon-silly-ation/issues/new"
    @echo ""
