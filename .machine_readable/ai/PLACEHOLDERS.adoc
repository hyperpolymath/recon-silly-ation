...
NOTE: recon-silly-ation has already been instantiated — its `{{PLACEHOLDER}}`
tokens have been substituted. This file is kept as historical reference for
the substitution mechanism `just init` used, not as a live checklist.
