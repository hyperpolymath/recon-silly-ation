...
# recon-silly-ation ABI/FFI Documentation
...
│  generated/abi/recon_silly_ation.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to librecon_silly_ation.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── recon_silly_ation.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── recon_silly_ation.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/recon_silly_ation.h
```
...
```c
#include "recon_silly_ation.h"

int main() {
    void* handle = recon_silly_ation_init();
    if (!handle) return 1;

    int result = recon_silly_ation_process(handle, 42);
    if (result != 0) {
        const char* err = recon_silly_ation_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    recon_silly_ation_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lrecon_silly_ation -L./zig-out/lib
```

### From Idris2

```idris
import ReconSillyAtion.ABI.Foreign
...
```rust
#[link(name = "recon_silly_ation")]
extern "C" {
    fn recon_silly_ation_init() -> *mut std::ffi::c_void;
    fn recon_silly_ation_free(handle: *mut std::ffi::c_void);
    fn recon_silly_ation_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = recon_silly_ation_init();
        assert!(!handle.is_null());

        let result = recon_silly_ation_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const librecon_silly_ation = "librecon_silly_ation"

function init()
    handle = ccall((:recon_silly_ation_init, librecon_silly_ation), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:recon_silly_ation_process, librecon_silly_ation), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:recon_silly_ation_free, librecon_silly_ation), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/recon_silly_ation.h
   ```
