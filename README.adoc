...
WASM document reconciliation, built around the ReconForth VM. It is an
`interop` component of the `reposystem` tool ecosystem (see
`reposystem/config/tools.ncl`), exposing a `reconcile` action.

== What this is

recon-silly-ation reconciles documents by running the ReconForth virtual
machine compiled to WebAssembly. It is registered in `reposystem` as an
`interop`-role tool with `status = 'extracted` (a gitlink submodule of
`reposystem`), whose sole declared action is `reconcile`.
